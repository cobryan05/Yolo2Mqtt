...
class StreamEventRecorder:
    ...
    def stopEventRecording(self, eventParams: MediaManager.EventParams):
        ...
        if recording is None:
            logger.warning(
                f"Failed to stopEventRecording. Event not found: {eventParams.eventName}"
            )
            return
...
class RecordingManager:
    def __init__(self, args: argparse.Namespace):
        ...
        self._mediaManager = MediaManager(
            recSettings.mediaRoot, daysToKeep=recSettings.keepVideosDays
        )
...
def parseArgs():
    ...
    parser.add_argument(
        "--dbgSkipExistingDelay",
        action="store_true",
        help="If true, do not overwrite existing delayed streams",
        default=False,
    )
